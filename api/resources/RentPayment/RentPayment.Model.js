...
import { PAYMENT_STATUS, PAYMENT_MODE, PLATFORM_FEE_PCT } from './RentPayment.Constant.js'
...
import { createOrder, verifyCheckoutSignature, verifyWebhookSignature, createPayoutToBank } from '../../helper/razorpay.js'
...
const computeDueMonth = (agreement, requestedMonth, requestedYear) => {
  ...
  let dueDate = dayjs(`${y}-${String(m).padStart(2, '0')}-${String(dueDay).padStart(2, '0')}`)
  ...
  if (dueDate.isBefore(agreementStart, 'day')) {
    ...
    m = next.month() + 1
    y = next.year()
    dueDate = dayjs(`${y}-${String(m).padStart(2, '0')}-${String(dueDay).padStart(2, '0')}`)
  }
...
const computePenaltyFromAgreement = (agreement, dueDate, asOf) => {
  ...
  const daysAfterDue = asOf.isAfter(dueDate, 'day') ? asOf.diff(dueDate, 'day') : 0
...
const getDueSummary = async (req, res) => {
  ...
  const pendingPayment = await rentPaymentModel.findOne({ agreementId, status: PAYMENT_STATUS.PENDING })
  if (pendingPayment) {
    const pMonth = pendingPayment.month
    const pYear = pendingPayment.year
    const pDueDay = agreement.paymentSchedule?.dueDay || 1
    const pDueDate = dayjs(`${pYear}-${String(pMonth).padStart(2, '0')}-${String(pDueDay).padStart(2, '0')}`)

    const priorPaid = await rentPaymentModel.findOne({ agreementId, status: 'paid' })
    const isFirstPayment = !priorPaid
    const securityDeposit = isFirstPayment ? Number(agreement.securityDeposit || 0) : 0
    ...
    return {
      ...
      totalAmount: pendingPayment.totalAmountCollected || (Number(agreement.rentAmount) + securityDeposit),
      alreadyPaid: false,
      paymentPending: true,
  ...
  const dueDay = agreement.paymentSchedule?.dueDay || 1
  while (await rentPaymentModel.findOne({ agreementId, month: m, year: y, status: PAYMENT_STATUS.PAID })) {
    const next = dueDate.add(1, 'month')
    m = next.month() + 1
    y = next.year()
    dueDate = dayjs(`${y}-${String(m).padStart(2, '0')}-${String(dueDay).padStart(2, '0')}`)
  ...
  const { penaltyAmount, daysLate, bufferDays } = computePenaltyFromAgreement(agreement, dueDate, asOf)
...
const createPaymentOrder = async ({ agreementId, userId, month, year }) => {
  const agreement = await rentalAgreementModel.findById(convertToObjectId(agreementId)).lean()
  if (!agreement) { const e = new Error('Agreement not found'); e.statusCode = 404; throw e }
  if (String(agreement.userId) !== String(userId)) { const e = new Error('Not authorized for this agreement'); e.statusCode = 403; throw e }
  ...
  const existingPending = await rentPaymentModel.findOne({ agreementId, status: PAYMENT_STATUS.PENDING })
  if (existingPending) { const e = new Error('A payment is already being processed. Please wait for it to complete or fail.'); e.statusCode = 409; throw e }
  ...
  const { penaltyAmount } = computePenaltyFromAgreement(agreement, dueDate, now)
  const rentAmount = Number(agreement.rentAmount)

  // Security deposit for first payment
  const priorPaid = await rentPaymentModel.findOne({ agreementId, status: PAYMENT_STATUS.PAID })
  const securityDeposit = !priorPaid ? Number(agreement.securityDeposit || 0) : 0

  const totalAmount = rentAmount + penaltyAmount + securityDeposit

  const { totalPaise, platformFeeAmount, ownerPayoutAmount } = computeFeeSplit(totalAmount)

  const transactionNumber = `TXN-${Date.now()}`
  const order = await createOrder({
    amountPaise: totalPaise,
    receipt: transactionNumber,
    notes: { agreementId: String(agreementId), month: m, year: y, userId: String(userId) }
  })
  ...
  const payment = await rentPaymentModel.findOneAndUpdate(filter, update, { upsert: true, new: true, setDefaultsOnInsert: true })
  ...
  return {
    ...
    paymentId: payment._id,
    breakup: { rentAmount, penaltyAmount, securityDeposit, totalAmount, platformFeeAmount, ownerPayoutAmount }
  }
}
...
const triggerOwnerPayout = async (payment) => {
  ...
  if (!bank?.accountNumber || !bank?.ifsc || !bank?.accountHolderName) {
    await rentPaymentModel.updateOne(
      { _id: payment._id },
      { $set: { payoutStatus: 'failed', 'metadata.payoutError': 'Owner bank details missing' } }
    )
    return
  }
  ...
  try {
    ...
    await rentPaymentModel.updateOne(
      { _id: payment._id },
      { $set: { razorpayPayoutId: payout.id, payoutStatus: payout.status || 'queued' } }
    )
  } catch (err) {
    await rentPaymentModel.updateOne(
      { _id: payment._id },
      { $set: { payoutStatus: 'failed', 'metadata.payoutError': err.response?.data?.error?.description || err.message } }
    )
  }
}

const verifyAndCapture = async ({ razorpay_order_id, razorpay_payment_id, razorpay_signature }) => {
  const valid = verifyCheckoutSignature({
    orderId: razorpay_order_id,
    paymentId: razorpay_payment_id,
    signature: razorpay_signature
  })
  if (!valid) { const e = new Error('Invalid signature'); e.statusCode = 400; throw e }

  const payment = await rentPaymentModel.findOne({ razorpayOrderId: razorpay_order_id })
  if (!payment) { const e = new Error('Payment row not found'); e.statusCode = 404; throw e }
  ...
  if (payment.status !== PAYMENT_STATUS.PAID) {
    ...
    triggerOwnerPayout(payment).catch(err => console.error('Payout error:', err.message))

    // Notify owner
    const agreement = await rentalAgreementModel.findById(payment.agreementId).lean()
    if (agreement) notifyOwnerOfPayment(payment, agreement).catch(err => console.error('Notify error:', err.message))
  }
...
const handleRazorpayWebhook = async ({ rawBody, signature }) => {
  ...
  if (type === 'payment.captured') {
    ...
    const payment = await rentPaymentModel.findOne({ razorpayOrderId: entity.order_id })
  ...
  } else if (type === 'payout.processed' || type === 'payout.failed' || type === 'payout.reversed') {
    const entity = event.payload.payout.entity
    const payoutStatus = type.split('.')[1]
    await rentPaymentModel.updateOne(
      { razorpayPayoutId: entity.id },
      { $set: { payoutStatus, 'metadata.payoutEvent': { at: new Date(), status: payoutStatus } } }
    )
  }
...
const notifyOwnerOfPayment = async (payment, agreement) => {
  try {
    ...
    // Look up tenant name
    const tenant = await userModel.findById(payment.userId).select('name').lean()
    ...
    await NotificationModel.createNotification({
      ...
      meta: { paymentId: payment._id, agreementId: payment.agreementId, tenantName },
...
const confirmOfflinePayment = async ({ agreementId, userId, month, year }) => {
  const agreement = await rentalAgreementModel.findById(convertToObjectId(agreementId)).lean()
  if (!agreement) { const e = new Error('Agreement not found'); e.statusCode = 404; throw e }
  if (String(agreement.userId) !== String(userId)) { const e = new Error('Not authorized'); e.statusCode = 403; throw e }
...
const getPaymentsByOwner = async (ownerId, options = {}) => {
  ...
  const cursor = rentPaymentModel.find(q)
